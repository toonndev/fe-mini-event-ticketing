...
export const BookingForm = ({ eventId, remainingTickets, usedQuota, isPast, maxTicketsPerUser, onSuccess }: BookingFormProps) => {
  const [apiError, setApiError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  const maxPerUser = maxTicketsPerUser ?? 5;
  const maxQty = Math.min(maxPerUser - usedQuota, remainingTickets);
  const isSoldOut = remainingTickets === 0;
  const isQuotaReached = usedQuota >= maxPerUser;
  ...
  const quotaPercent = (usedQuota / maxPerUser) * 100;
        ...
        <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <Box mb={2}>
            <Typography variant="body2" color="text.secondary" mb={0.5}>
              My bookings: {usedQuota} / {maxPerUser} used
            </Typography>
            <LinearProgress
              variant="determinate"
              value={quotaPercent}
              color={usedQuota >= maxPerUser ? 'error' : 'primary'}
